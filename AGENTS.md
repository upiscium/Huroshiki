...
- `url_artifacts.py` owns self-hosted URL validation, DNS-pinned public-network transport, bounded downloads, JAR identity parsing, URL logs, and Packwiz metadata writes; `portable_paths.py` owns generated metadata path/filename portability; `huroshiki_core.py` reexports the URL API and coordinates it with transactions and project configuration.
...
- Content overlays must not contain symlinks or Packwiz-owned names (`pack.toml`, `index.toml`, or `*.pw.toml`). Keep overlay inspection and safe child resolution centralized in `overlay_policy.py` and never follow links while validating, listing, or copying overlays.
