...
- Settings mutations in `packctl.py` pin the project directory, parse committed and local YAML from descriptor snapshots, validate the prospective merged configuration, and publish with Linux `renameat2` CAS. Do not replace this with path-based reopen/check/replace logic or an implicit non-atomic fallback.
