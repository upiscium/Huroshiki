...
class ProviderLookupHelperTest(unittest.TestCase):
    @staticmethod
    def version(*, artifact_id="Abcd1234", project_id="Proj0001", release_type="release", date="2026-01-01T00:00:00Z", files=None, **overrides):
        value = {
            "id": artifact_id,
            ...
            "loaders": ["fabric"],
            "files": files if files is not None else [{"filename": "mod.jar", "primary": True}],
        }
        value.update(overrides)
        return value

    def test_modrinth_versions_filters_and_sorts_strict_candidates(self) -> None:
        seen = {}
        payload = [
            self.version(artifact_id="Zzzz9999", date="2026-01-01T00:00:00+00:00"),
            self.version(artifact_id="Aaaa0000", date="2026-01-01T00:00:00+00:00", release_type="beta"),
            self.version(artifact_id="Bbbb1111", date="2026-02-01T00:00:00+00:00"),
        ]
        ...
        self.assertEqual(set(result[0]), {"provider", "project_id", "artifact_id", "version", "filename", "game_versions", "loaders", "release_type", "published_at"})

    def test_modrinth_versions_include_prerelease_and_tie_break(self) -> None:
        payload = [
            self.version(artifact_id="Zzzz9999", release_type="alpha"),
            self.version(artifact_id="Aaaa0000", release_type="beta"),
        ]
        with patch.object(provider_lookup, "urlopen", return_value=FakeResponse(json.dumps(payload).encode())):
            result = provider_lookup.modrinth_versions(
                "Proj0001", minecraft="1.21.1", loader="fabric", include_prerelease=True
            )
    ...
    def test_modrinth_versions_rejects_malformed_candidates(self) -> None:
        cases = [
            ...
            [self.version(artifact_id="bad")],
            [self.version(date="2026-01-01T00:00:00")],
            [self.version(release_type="unknown")],
        ...
        for payload in cases:
            with self.subTest(payload=payload), patch.object(
                provider_lookup, "urlopen", return_value=FakeResponse(json.dumps(payload).encode())
            ):
    ...
    def test_modrinth_versions_rejects_duplicate_json_and_artifact_ids(self) -> None:
        duplicate_json = b'[{"id":"Abcd1234","id":"Efgh5678"}]'
        with patch.object(provider_lookup, "urlopen", return_value=FakeResponse(duplicate_json)):
            with self.assertRaises(provider_lookup.LookupError):
                provider_lookup.modrinth_versions("Proj0001", minecraft="1.21.1", loader="fabric")
        payload = [self.version(), self.version(date="2026-01-02T00:00:00Z")]
        with patch.object(provider_lookup, "urlopen", return_value=FakeResponse(json.dumps(payload).encode())):
    ...
    def test_modrinth_versions_rejects_provider_record_and_byte_overflow(self) -> None:
        payload = [
            self.version(artifact_id="Abcd1234"),
            self.version(artifact_id="Efgh5678"),
        ]
        with patch.object(
            provider_lookup,
            "_MAX_PROVIDER_VERSION_RECORDS",
            1,
        ), patch.object(
            provider_lookup,
            "urlopen",
            ...
            with self.assertRaisesRegex(provider_lookup.LookupError, "too many"):
                provider_lookup.modrinth_versions(
                    "Proj0001",
                    minecraft="1.21.1",
                    loader="fabric",
